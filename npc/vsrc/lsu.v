...
// /************ Passthrough signals to next stages ************/
// endmodule
/************ Passthrough signals to next stages ************/
    // to BRU
    assign o_lsu_imm      = i_lsu_imm;
    assign o_lsu_pc       = i_lsu_pc;
    assign o_lsu_rs1      = i_lsu_rs1;
    assign o_lsu_is_jal   = i_lsu_is_jal;
    assign o_lsu_is_jalr  = i_lsu_is_jalr;
    assign o_lsu_brch     = i_lsu_brch;
    assign o_lsu_csr_npc  = i_lsu_csr_npc;
    assign o_lsu_is_ejump = i_lsu_is_mret | i_lsu_is_ecall;

    // to WEU
    assign o_lsu_rd_id    = i_lsu_rd_id;
    assign o_lsu_gpr_wen  = i_lsu_gpr_wen;
    assign o_lsu_csr_wid  = i_lsu_csr_wid;
    assign o_lsu_csr_rd   = i_lsu_csr_rd;
    assign o_lsu_csr_wen  = i_lsu_csr_wen;
    assign o_lsu_is_mret  = i_lsu_is_mret;
    assign o_lsu_is_ecall = i_lsu_is_ecall;

/************ The NEW, SAFE Memory Access Logic ************/

    // 这个寄存器用于存放从内存读出并经过处理（符号位/零扩展）后的数据
    reg [`CPU_Bus] lsu_mem_rdata_reg;

    // 这个组合逻辑块用于准备写操作的掩码(wmask)。
    // 它是安全的，因为它不包含任何DPI调用。
    wire [7:0] wmask;
    assign wmask = (i_lsu_is_store == `TRUE && i_lsu_func3 == `INST_SB) ? `WByte :
                   (i_lsu_is_store == `TRUE && i_lsu_func3 == `INST_SH) ? `WHalf :
                   (i_lsu_is_store == `TRUE && i_lsu_func3 == `INST_SW) ? `WWord :
                   8'b0; // 默认值

    // 这是处理所有内存交互的核心时序逻辑块。
    // 将DPI调用放在这里，可以确保它们只在时钟边沿触发，且只在其使能信号有效时执行。
    // 这可以从根本上防止仿真器进行任何“投机性”的非法调用。
    always @(posedge clk) begin
        if (rst) begin
            lsu_mem_rdata_reg <= 0;
        end else begin
            ...
            if (i_lsu_is_load == `TRUE) begin
                // 步骤1: 安全地调用DPI函数从内存获取原始数据。
                // 这个调用现在被 i_lsu_is_load 信号严格保护。
                automatic logic [`CPU_Bus] raw_read_data = dmem_read(i_lsu_exu_res);

                // 步骤2: 根据指令类型(lw, lhu, lb等)处理原始数据，并存入寄存器。
                case (i_lsu_func3)
                    `INST_LBU: lsu_mem_rdata_reg <= {24'd0, raw_read_data[7:0]};
                    `INST_LHU: lsu_mem_rdata_reg <= {16'd0, raw_read_data[15:0]};
                    `INST_LB:  lsu_mem_rdata_reg <= {{24{raw_read_data[7]}}, raw_read_data[7:0]};
                    `INST_LH:  lsu_mem_rdata_reg <= {{16{raw_read_data[15]}}, raw_read_data[15:0]};
                    `INST_LW:  lsu_mem_rdata_reg <= raw_read_data;
            ...
            if (i_lsu_is_store == `TRUE) begin
                // 这个调用现在被 i_lsu_is_store 信号严格保护。
                pmem_write(i_lsu_exu_res, i_lsu_rs2, wmask);
            end
        end
    end

    // 最终输出到写回阶段(WEU)的数据选择器。
    // 它在ALU结果(用于绝大多数指令)和从内存读取的数据(用于load指令)之间进行选择。
    assign o_lsu_rd = (i_lsu_is_load == `TRUE) ? lsu_mem_rdata_reg : i_lsu_exu_res;


    /************ Handshake logic (unchanged) ************/
    // Note: This simple handshake might need adjustment for a pipelined design,
    // but it is kept as is from the original structure.
    reg post_valid_reg;
    always @(posedge clk) begin
        if(rst == 1'b1)
            post_valid_reg <= 1'b0;
