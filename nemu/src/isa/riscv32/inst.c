...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = (word_t)(uint16_t)Mr(src1 + imm, 2));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1;
                                                                  #ifdef CONFIG_FTRACE
                                                                    // 识别RET指令：jalr x0, x1, 0 (机器码0x00008067)
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, printf("csrrw executed\n");R(rd) = CSR(imm); CSR(imm) = src1);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, {R(rd) = s->pc + 4; s->dnpc = s->pc + imm;
                                                                #ifdef CONFIG_FTRACE
                                                                  if (rd == 1) { // 当目标寄存器是ra(x1)时，表示函数调用
                                                                    ftrace_call(s->pc, s->dnpc); 
                                                                  }
                                                                #endif
                                                                });

  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if (src1 == src2) s->dnpc = s->pc + imm; else s->dnpc = s->snpc);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if (src1 != src2) s->dnpc = s->pc + imm; else s->dnpc = s->snpc);
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, printf("ecall executed\n");s->dnpc = isa_raise_intr(11, s->pc));//自陷指令
